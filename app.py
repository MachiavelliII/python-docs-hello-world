...
def send_discord_alert(method, ip, body_preview):
    ...
    payload = {
        "embeds": [
            {
                "title": f"📡 Incoming Request — {method} /api/v1/",
                "color": 0x57F287,
                "fields": [
...
@app.route("/api/v1/", methods=["GET", "POST", "PUT", "PATCH", "DELETE", "HEAD", "OPTIONS"])
def api_v1():
    method       = request.method
    ip           = get_client_ip()
    body_preview = request.get_data(as_text=True)[:150]

    logger.info("method=%s ip=%s body=%r", method, ip, body_preview)
    send_discord_alert(method, ip, body_preview)

    return jsonify({"status": "ok", "method": method}), 200
